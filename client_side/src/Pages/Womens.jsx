  ...
  const fetchdata = async () => {
    ...
    try {
      const response = await axios({
        url: "https://myntra-backend-ra9i.onrender.com/api/womens/products",
        method: "GET",
      });
      ...
      <div id="mens-products">
        ...
        {Array.isArray(sortedData) &&
          sortedData.map((product) => (
            <div className="mens-items" key={product.id || product.title}>
              <img src={product.img} alt={product.title} />
              <p><b>{product.title}</b></p>
              <h2>
                <b>Rs </b>{product.price} <span style={{ color: "gold" }}>({product.discount} OFF)</span>
              </h2>
              <span style={{ display: "inline-flex", alignItems: "center", gap: "4px", color: "gold" }}>
                <FaRegStar />
                {product.review}
              </span>
